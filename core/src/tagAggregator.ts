...
import { callOpenAICompatible, resolveBaseUrl, OpenAICompatPayload } from "./llm/openaiCompat";
import type { TagCategory, TagMetadata } from "./routingStore";
import { localNormalize } from "./tagNormalizer";

export { createEmptyTagMetadata } from "./routingStore";

/**
 * Result from a single LLM aggregation batch
 */
export type AggregationResult = {
    groups: Record<string, string[]>;  // canonical_tag -> tags
};
...
function isAggregationEnabled(): boolean {
...
function pickRandomCategories(categories: string[], limit: number): string[] {
...
export function findUnknownTags(
    ...
    knownCategories?: Record<string, TagCategory>
): string[] {
...
function buildAggregationPrompt(
    ...
    const categoriesForPrompt = pickRandomCategories(
        existingCategories,
        MAX_CATEGORY_CONTEXT
    );
 ...
 */
function parseAggregationResponse(text: string): AggregationResult | null {
    ...
    const normalizeGroups = (raw: Record<string, unknown>): Record<string, string[]> => {
    ...
    const groupsFromMappings = (raw: Record<string, unknown>): Record<string, string[]> => {
        const groups: Record<string, string[]> = {};
    ...
    try {
        ...
        if ("groups" in parsed && parsed.groups && typeof parsed.groups === "object") {
            groups = normalizeGroups(parsed.groups as Record<string, unknown>);
        } else if ("mappings" in parsed && parsed.mappings && typeof parsed.mappings === "object") {
            groups = groupsFromMappings(parsed.mappings as Record<string, unknown>);
        } else {
 ...
 */
export async function aggregateTagsBatch(
    unknownTags: string[],
    existingCategories: string[]
    ...
    const buildIdentityGroups = (tags: string[]): Record<string, string[]> => {
        const groups: Record<string, string[]> = {};
        for (const tag of tags) {
            if (!tag) {
                continue;
    ...
    const isPlaceholderOnly = (groups: Record<string, string[]>): boolean => {
    ...
    const systemPrompt =
        "你是代码标签聚类助手。仅同义/翻译/格式变体合并(中英同义可合并);其他保留或新建;不泛化;不得丢标签;类别名可中文或 snake_case 英文;禁用占位名 category/other/misc;只输出 JSON;偏好中文。";
        ...
        if (text) {
            ...
            if (result) {
                if (isPlaceholderOnly(result.groups)) {
                    console.warn(
                        "[tagAggregator] LLM returned placeholder categories, falling back to identity mapping."
                    );
                } else {
                return result;
                }
 ...
 */
export async function aggregateAllUnknownTags(
    unknownTags: string[],
    ...
    if (!isAggregationEnabled()) {
        const categories: Record<string, TagCategory> = { ...metadata.categories };
    ...
    const updatedMetadata: TagMetadata = {
        aliases: { ...metadata.aliases },
    ...
    await runWithConcurrency(batches, Math.min(concurrency, totalBatches), async (item) => {
        ...
        if (isFirst) {
            console.log(
                `[tagAggregation] 开始第一次聚合 batch=${batchIndex}/${totalBatches} tags=${batch.length} totalTags=${unknownTags.length}`
            );
        } else if (isSecond) {
            console.log(
                `[tagAggregation] 开始第二次聚合 batch=${batchIndex}/${totalBatches} tags=${batch.length} totalTags=${unknownTags.length}`
            );
        }
        ...
        applyChain = applyChain.then(() => {
            ...
            console.log(
                `[tagAggregation] 聚合结果 batch=${batchIndex}/${totalBatches} groups=${groupCount} detail=${JSON.stringify(result.groups)}`
            );

            for (const [canonicalRaw, tags] of Object.entries(result.groups)) {
                const canonical =
                    localNormalize(canonicalRaw) ?? canonicalRaw.trim().toLowerCase();
                if (!canonical) {
                    continue;
                }
                ...
                for (const rawTag of list) {
                    ...
                    const normalizedTag =
                        localNormalize(rawTag) ?? rawTag.trim().toLowerCase();
            ...
            if (isFirst) {
                console.log(
                    `[tagAggregation] 结束第一次聚合 batch=${batchIndex}/${totalBatches} groups=${groupCount} aliases=${Object.keys(updatedMetadata.aliases).length}`
                );
            } else if (isSecond) {
                console.log(
                    `[tagAggregation] 结束第二次聚合 batch=${batchIndex}/${totalBatches} groups=${groupCount} aliases=${Object.keys(updatedMetadata.aliases).length}`
                );
            }
        });
    });
 ...
 */
export function ensureCategoriesForTags(
    metadata: TagMetadata,
 ...
 */
export function cleanupUnusedAliases(metadata: TagMetadata): TagMetadata {
    const cleanedAliases: Record<string, string> = {};
    const cleanedCategories: Record<string, TagCategory> = {};
 ...
 */
export function mergeTagMetadata(
    existing: TagMetadata,
    incoming: TagMetadata
): TagMetadata {
    return {
