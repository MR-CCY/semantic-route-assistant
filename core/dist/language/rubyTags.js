...
exports.inferRubyBaseTags = inferRubyBaseTags;
 ...
 */
function inferRubyBaseTags(input) {
    ...
    if (signature) {
        if (/\bclass\b/.test(signature))
            tags.add("class");
        if (/\bmodule\b/.test(signature))
            tags.add("module");
        if (/\bdef\b/.test(signature))
            tags.add("method");
        if (/\bself\.\w+/.test(signature))
        ...
        if (/\bprotected\b/.test(signature))
            tags.add("protected");
        // Rails specific
        if (/\bhas_many\b/.test(signature))
            tags.add("association");
        if (/\bbelongs_to\b/.test(signature))
            tags.add("association");
        if (/\bhas_one\b/.test(signature))
            tags.add("association");
        if (/\bvalidates\b/.test(signature))
            tags.add("validation");
        if (/\bbefore_action\b/.test(signature))
            tags.add("callback");
        if (/\bafter_action\b/.test(signature))
            tags.add("callback");
