...
exports.generateBriefAndTagsForSymbol = generateBriefAndTagsForSymbol;
...
function extractImplementationBody(input) {
...
function adjustTagsForTrivialImplementation(impl, tags) {
    if (!impl) {
        return tags;
    }
...
function getTrivialBriefAndTags(input) {
    ...
    const isMemberReturn = expr.includes("this.") ||
        expr.includes("this->") ||
...
const DEFAULT_SYSTEM_PROMPT = "你是一位拥有 20 年经验的资深代码架构师和算法专家。" +
...
function renderUserPrompt(input, implNote) {
...
async function generateBriefAndTagsForSymbol(input) {
    ...
    try {
        ...
        if (!resolvedBaseUrl) {
            console.warn(`[generateBriefForSymbol] 未配置 SRCA_LLM_BASE_URL，无法调用 ${provider}。`);
            return { brief: `自动生成描述: ${input.signature}`, tags: [] };
        }
        console.log(`[generateBriefForSymbol] LLM prompt file=${input.filePath ?? "unknown"} signature=${input.signature}`);
                ...
                try {
                    ...
                    if (parsed?.brief) {
                        return {
                            ...
                            tags: adjustTagsForTrivialImplementation(input.implementation, normalizeTags(parsed.tags || []))
                        };
                    }
                }
                catch {
        ...
        console.warn(`[generateBriefForSymbol] 调用失败: ${provider} ${input.moduleName} ${input.signature}`, error);
    }
    return { brief: `自动生成描述: ${input.signature}`, tags: [] };
}
